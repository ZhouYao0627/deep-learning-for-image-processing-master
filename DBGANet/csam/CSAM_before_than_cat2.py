...
class spatial_attention(nn.Module):
    ...
    def __init__(self, kernel_size=7):
        ...
        self.conv1 = nn.Conv2d(in_channels=4416, out_channels=1, kernel_size=kernel_size, padding=padding, bias=False)

        # 反卷积复原特征图大小 [b,1,h/2,w/2]==>[b,1,h,w]
        self.deconv = nn.ConvTranspose2d(in_channels=1, out_channels=1, kernel_size=1, stride=2, padding=0, bias=False)
        # sigmoid函数
        self.sigmoid = nn.Sigmoid()
        ...
        self.logit = nn.Sequential(
            OrderedDict((
                ('conv', nn.Conv2d(2208, 2208, 3, padding=1, bias=False)),
                ('bn', nn.InstanceNorm2d(2208, affine=True)),
                ('gate', SoftGate()),
            ))
        )
