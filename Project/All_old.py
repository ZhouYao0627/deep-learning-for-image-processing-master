...
def createJson(file_u, file_v, file_gust):
    if 0 < i < 10:
        file_path = r'E:\Project\nc_data\{}\gfs.t00z.pgrb2.0p25.f00{}.nc'.format(today, i)
        ...
        json_gust = gen_data_dict(gust, parameterNumber0, parameterNumberName1, dx, dy, la1, la2, nx, ny, lo1, lo2)
        print("json_gust success" + str(i))

        u = np.squeeze(ds['u-gwd'][:])
        parameterNumber1 = 1
        parameterNumberName1 = 'U component of wind'
        json_u = gen_data_dict(u, parameterNumber1, parameterNumberName1, dx, dy, la1, la2, nx, ny, lo1, lo2)
        print("json_u success" + str(i))

        v = np.squeeze(ds['v-gwd'][:])
        parameterNumber2 = 2
        parameterNumberName2 = 'v component of wind'
        json_v = gen_data_dict(v, parameterNumber2, parameterNumberName2, dx, dy, la1, la2, nx, ny, lo1, lo2)
        ...
        with open(file_gust + '\\windJsonGust{}.json'.format(name2), 'w') as fp:
            json.dump(json_gust, fp)
        with open(file_u + '\\windJsonU{}.json'.format(name2), 'w') as fp:
            json.dump(json_u, fp)
        with open(file_v + '\\windJsonV{}.json'.format(name2), 'w') as fp:
            json.dump(json_v, fp)
        print('JsonSuccess: ' + str(i))
    if 10 <= i < 100:
        file_path = r'E:\Project\nc_data\{}\gfs.t00z.pgrb2.0p25.f0{}.nc'.format(today, i)
...
def gen_data_dict(data_array, parameterNumber, parameterNumberName, dx, dy, la1, la2, nx, ny, lo1, lo2):
    header = {
        "parameterUnit": "m/s",
...
# 整合了下载数据、将gfs数据转成nc、创建图像和创建json文件
if __name__ == '__main__':
    ...
    # ============4. 创建json文件============
    today = datetime.date.today()  # 2022-12-16
    file_u = "E:\\Project\\json\\{}\\windU".format(today)
    file_v = "E:\\Project\\json\\{}\\windV".format(today)
    file_gust = "E:\\Project\\json\\{}\\windGust".format(today)
    mkdir(file_u)
    mkdir(file_v)
    mkdir(file_gust)

    for i in range(121):
        createJson(file_u, file_v, file_gust)
    print("-----------------createJson success-----------------")

    endtime = datetime.datetime.now()
    print("endtime_createJson", endtime)
    print("endtime - starttime_createJson", endtime - starttime)
