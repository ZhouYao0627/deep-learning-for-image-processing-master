...
def save_data(url, FILENAME):
    try:
        ...
        for chunk in result.iter_content(1000 * 1024):
            buff += chunk
            t2 = time.time()
            print(url, len(buff), all_size, len(buff) / all_size, len(buff) / (t2 - t1), t2 - t1,
                  (t2 - t1) / len(buff) * (all_size - len(buff)))
        f = open(FILENAME, 'wb')
        f.write(buff)
        f.close()
...
def run_it():
    ...
    for year in root_year:
        for month in root_mouth:
            for day in root_day:
                for start_time in ['00', '06', '12', '18']:
                    # for start_time in ['00','06']:
                    for forecast_time in range(0, 121):
                        save_path = root_save_path + '/' + str(year) + str(month).zfill(2) + str(day).zfill(
                            2) + "_" + start_time + "_atmos/"
                        if not os.path.exists(save_path):
                            os.mkdir(save_path)
                        date = str(year) + str(month).zfill(2) + str(day).zfill(2)
                        file_name = "gfs.t" + start_time + "z.pgrb2.0p25.f" + str(forecast_time).zfill(3)
                        data_file = os.path.join(save_path, file_name)
                        # a = save_path+file_name
