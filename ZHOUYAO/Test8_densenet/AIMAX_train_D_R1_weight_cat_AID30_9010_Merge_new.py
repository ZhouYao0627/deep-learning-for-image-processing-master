...
# AID30_50_50_new_2dropout: best_acc:

# 2dropout: best: 0.945
# 0dropout: best: 0.941
# D_R_AID30_9010_Merge_new.pth是加了2dropout
# D_R_AID30_9010_Merge_new1.pth是未加dropout
def main(args):
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    ...
    epochs = 100
    best_acc = 0.0
    save_path = './save_weights/D_R_AID30_9010_Merge_new.pth'
    ...
    for epoch in range(epochs):
        ...
        with open('./Draw/train_accurate_AID30_9010_Merge_new_0dropout.txt', 'a+') as f:
            f.write(str(train_accurate) + ',')

        with open('./Draw/val_accurate_AID30_9010_Merge_new_0dropout.txt', 'a+') as f:
            f.write(str(val_accurate) + ',')

        train_loss1 = running_loss / train_steps
        with open('./Draw/train_loss_AID30_9010_Merge_new_0dropout.txt', 'a+') as f:
