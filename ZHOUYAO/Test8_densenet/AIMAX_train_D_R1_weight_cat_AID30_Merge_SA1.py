...
from CSAM_before_than_cat1_new import csam as csam1
from CSAM_before_than_cat2_mew import csam as csam2
...
class mergeblock3(nn.Module):
    def __init__(self):
        ...
        self.sa2 = Self_Attn(2048, 'Relu')

        self.conv_1x1 = nn.Conv2d(in_channels=4096, out_channels=2048, kernel_size=1, padding=0, bias=False)
    ...
    def forward(self, InData1, InData2):
        ...
        # 第二阶段（方法一）两分支分别sa  Merge_sa1.txt  best_acc 0.9386
        x1, _ = self.sa1(fea1)
        x2, _ = self.sa2(fea2)
        result = torch.cat((x1, x2), dim=1)  # # 7*7*4096

        # 第二阶段（方法二）两分支分别融合融然后sa  best_acc 0.936
        # x1 = torch.add(fea1, fea2)
        # x2 = torch.add(fea1, fea2)
        # x3, _ = self.sa1(x1)
        # x4, _ = self.sa2(x2)
        # result = torch.cat((x3, x4), dim=1)  # 7*7*4096

        # 第二阶段（方法三）两分支分别concat然后sa  best_acc 0.9354
        # x1 = torch.cat((fea1, fea2), dim=1)
        # x2 = torch.cat((fea1, fea2), dim=1)
        # x3, _ = self.sa1(x1)
        # x4, _ = self.sa2(x2)
        # result = torch.cat((x3, x4), dim=1)  # 7*7*8192 -> EF里需修改一下

        # 第二阶段（方法四）两分支分别concat和融合然后sa  # best_acc 0.9334
        # x1 = torch.add(fea1, fea2)
        # x2 = torch.cat((fea1, fea2), dim=1)
        # x3, _ = self.sa1(x1)  # 7*7*2048
        # x4, _ = self.sa2(x2)  # 7*7*4096
        # x4 = self.conv_1x1(x4)  # 7*7*2048
        # result = torch.cat((x3, x4), dim=1)  # 7*7*4096

        # 第二阶段（方法五）两分支先concat然后sa。 1个sa
        # 第二阶段（方法六）两分支先融合然后sa，1个sa
...
class EF(nn.Module):
    ...
    def __init__(self, net1, net2):
        ...
        self.conv_1x1_1 = nn.Conv2d(in_channels=2208, out_channels=2048, kernel_size=1, padding=0, bias=False)
        self.conv_1x1_2 = nn.Conv2d(in_channels=4096, out_channels=1024, kernel_size=1, padding=0, bias=False)
...
def main(args):
    ...
    epochs = 100
    best_acc = 0.0
    save_path = './D_R_cat_twocsam_drop05_2_AID30_Merge_1.pth'
    ...
    for epoch in range(epochs):
        ...
        with open('./Draw/train_accurate_D_R_cat_twocsam_AID30_Merge_sa4.txt', 'a+') as f:
            f.write(str(train_accurate) + ',')

        with open('./Draw/val_accurate_D_R_cat_twocsam_AID30_Merge_sa4.txt', 'a+') as f:
            f.write(str(val_accurate) + ',')

        train_loss1 = running_loss / train_steps
        with open('./Draw/train_loss_D_R_cat_twocsam_AID30_Merge_sa4.txt', 'a+') as f:
