...
class mergeblock(nn.Module):
    ...
    def forward(self, InData1, InData2):
        ...
        # 第一阶段
        max_out1 = self.avg_pool(InData1)
...
class EF(nn.Module):
    ...
    def __init__(self, net1, net2):
        ...
        self.fc = nn.Sequential(nn.Linear(1024, 19))
...
# OPTIMAl_80_20_2dropout: best_acc:
def main(args):
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    ...
    image_path = os.path.join(data_root, "data_set", "RS", "80_20")  # flower data set path
    ...
    with open('class_indices_OPTIMAL31.json', 'w') as json_file:
    ...
    epochs = 100
    best_acc = 0.0
    save_path = './save_weights/D_R_OPTIMAl31_8020_Merge_2dropout.pth'
    ...
    for epoch in range(epochs):
        ...
        with open('./Draw/train_accurate_OPTIMAL_8020_Merge_2dropout.txt', 'a+') as f:
            f.write(str(train_accurate) + ',')

        with open('./Draw/val_accurate_OPTIMAL_8020_Merge_2dropout.txt', 'a+') as f:
            f.write(str(val_accurate) + ',')

        train_loss1 = running_loss / train_steps
        with open('./Draw/train_loss_OPTIMAL_8020_Merge_2dropout.txt', 'a+') as f:
