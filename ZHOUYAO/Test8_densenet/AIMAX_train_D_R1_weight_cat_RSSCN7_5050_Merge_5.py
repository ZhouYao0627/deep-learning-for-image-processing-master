...
class R2D(nn.Module):
    def __init__(self):
        ...
        # self.conv_3x3_1 = nn.Conv2d(in_channels=2048, out_channels=2048, kernel_size=3, stride=1, padding=1, bias=False)
        # self.conv_3x3_2 = nn.Conv2d(in_channels=2208, out_channels=2208, kernel_size=3, stride=1, padding=1, bias=False)
    ...
    def forward(self, Input1, Input2):
        ...
        x1 = self.conv1_1_dw(Input1)
        x1 = self.conv1_1_pw(x1)
        x2 = self.conv2_1_dw(Input2)
...
class D2R(nn.Module):
    ...
    def forward(self, Input1, Input2):
        ...
        # x1 = self.conv_3x3_1(Input1)
        # x2 = self.conv_3x3_2(Input2)
...
class mergeblock2(nn.Module):
    ...
    def forward(self, InData1, InData2):
        ...
        # result = torch.cat([fea1, fea2], dim=1)

        # 第三阶段
        x1 = self.R2D.forward(fea1, fea2)
        x2 = self.D2R.forward(fea1, fea2)
        # print("x1.shape", x1.shape)  # [64, 2048, 7, 7]
        # print("x2.shape", x2.shape)  # [64, 2208, 7, 7]

        # concat拼接
        result = torch.cat([x1, x2], dim=1)  # 7*7*4256
...
def main(args):
    ...
    epochs = 100
    best_acc = 0.0
    save_path = './save_weights/D_R_cat_twocsam_drop05_2_RSSCN7_5050_Merge_1.pth'
    ...
    for epoch in range(epochs):
        ...
        with open('./Draw/train_accurate_D_R_cat_twocsam_RSSCN7_5050_Merge_1.txt', 'a+') as f:
            f.write(str(train_accurate) + ',')

        with open('./Draw/val_accurate_D_R_cat_twocsam_RSSCN7_5050_Merge_1.txt', 'a+') as f:
            f.write(str(val_accurate) + ',')

        train_loss1 = running_loss / train_steps
        with open('./Draw/train_loss_D_R_cat_twocsam_RSSCN7_5050_Merge_1.txt', 'a+') as f:
