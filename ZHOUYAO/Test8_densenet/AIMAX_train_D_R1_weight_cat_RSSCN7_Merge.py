...
class R2D(nn.Module):
    def __init__(self):
        ...
        self.conv1_1 = nn.Sequential(
            nn.Conv2d(in_channels=2048, out_channels=2048 * 2, kernel_size=1, padding=0, bias=False),
            nn.ReLU(),
            nn.BatchNorm2d(2048 * 2),
        )
        self.conv1_2 = nn.Sequential(
            nn.Conv2d(in_channels=2048 * 2, out_channels=2048 * 2, kernel_size=3, stride=1, padding=1, bias=False),
            nn.ReLU(),
            nn.BatchNorm2d(2048 * 2),
        )
        self.conv1_3 = nn.Sequential(
            nn.Conv2d(in_channels=2048 * 2, out_channels=2048, kernel_size=1, padding=0, bias=False),
            nn.ReLU(),
            nn.BatchNorm2d(2048),
        )

        self.conv2_1 = nn.Sequential(
            nn.Conv2d(in_channels=2208, out_channels=2208 * 2, kernel_size=1, padding=0, bias=False),
            nn.ReLU(),
            nn.BatchNorm2d(2208 * 2),
        )
        self.conv2_2 = nn.Sequential(
            nn.Conv2d(in_channels=2208 * 2, out_channels=2208 * 2, kernel_size=3, stride=1, padding=1, bias=False),
            nn.ReLU(),
            nn.BatchNorm2d(2208 * 2),
        )
        self.conv2_3 = nn.Sequential(
            nn.Conv2d(in_channels=2208 * 2, out_channels=2208, kernel_size=1, padding=0, bias=False),
            nn.ReLU(),
            nn.BatchNorm2d(2208),
        )
...
class mergeblock(nn.Module):
    ...
    def forward(self):
        ...
        # 第一次
        x3 = self.R2D.forward(x1, x2)
        x4 = self.D2R.forward(x1, x2)

        # 第二次
        x5 = self.R2D.forward(x3, x4)
        x6 = self.D2R.forward(x3, x4)

        # 第三次
        x7 = self.R2D.forward(x5, x6)  # 7*7*2048
        x8 = self.D2R.forward(x5, x6)  # 7*7*2208

        # concat拼接
        x = torch.cat([x7, x8], dim=1)  # 7*7*4256
...
def main(args):
    ...
    epochs = 100
    best_acc = 0.0
    save_path = './D_R_cat_twocsam_drop05_2_RSSCN7_Merge_1.pth'
    ...
    for epoch in range(epochs):
        ...
        with open('./Draw/train_accurate_D_R_cat_twocsam_RSSCN7_Merge_1.txt', 'a+') as f:
            f.write(str(train_accurate) + ',')

        with open('./Draw/val_accurate_D_R_cat_twocsam_RSSCN7_Merge_1.txt', 'a+') as f:
            f.write(str(val_accurate) + ',')

        train_loss1 = running_loss / train_steps
        with open('./Draw/train_loss_D_R_cat_twocsam_RSSCN7_Merge_1.txt', 'a+') as f:
