...
class EF(nn.Module):
    ...
    def __init__(self, net1, net2):
        ...
        self.conv1 = nn.Conv2d(in_channels=2208, out_channels=2048, kernel_size=1, stride=1, bias=False)
        self.bilinear_pooling = bilinear_pooling()

        self.avgpool = nn.Sequential(nn.AdaptiveAvgPool2d((1, 1)))
        self.dropout = nn.Dropout(0.5)
        self.fc = nn.Sequential(nn.Linear(2048, 7))
...
def main(args):
    ...
    epochs = 100
    best_acc = 0.0
    save_path = './D_R_cat_twocsam_drop05_2_RSSCN7_bp.pth'
    ...
    for epoch in range(epochs):
        ...
        with open('./Draw/train_accurate_D_R_cat_twocsam_RSSCN7_bp_1.txt', 'a+') as f:
            f.write(str(train_accurate) + ',')

        with open('./Draw/val_accurate_D_R_cat_twocsam_RSSCN7_bp_1.txt', 'a+') as f:
            f.write(str(val_accurate) + ',')

        train_loss1 = running_loss / train_steps
        with open('./Draw/train_loss_D_R_cat_twocsam_RSSCN7_bp_1.txt', 'a+') as f:
