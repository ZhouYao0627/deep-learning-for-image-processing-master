...
# 80_20: best_acc: 0.9738095238095238
def main(args):
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    ...
    epochs = 100
    best_acc = 0.0
    save_path = './save_weights/D_R_UCM21_9010_Merge_new.pth'
    ...
    for epoch in range(epochs):
        ...
        for step, data in enumerate(train_bar):
            ...
            loss = weight * loss_function1(logits, labels.to(device)) + (1 - weight) * loss_function2(logits,
                                                                                                      labels.to(device))
            loss.backward()
            optimizer.step()
            # CosineLR.step()
        ...
        print('[epoch %d] train_loss: %.3f  train_accuracy: %.3f  val_accuracy: %.3f  weight: %.3f' %
              (epoch + 1, running_loss / train_steps, train_accurate, val_accurate, weight))

        if val_accurate > best_acc:
            best_acc = val_accurate
            torch.save(net.state_dict(), save_path)

        with open('./Draw/train_accurate_UCM21_8020_Merge_new1.txt', 'a+') as f:
            f.write(str(train_accurate) + ',')

        with open('./Draw/val_accurate_UCM21_8020_Merge_new1.txt', 'a+') as f:
            f.write(str(val_accurate) + ',')

        train_loss1 = running_loss / train_steps
        with open('./Draw/train_loss_UCM21_8020_Merge_new1.txt', 'a+') as f:
