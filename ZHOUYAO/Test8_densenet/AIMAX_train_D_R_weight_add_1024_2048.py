...
class mergeblock(nn.Module):
    ...
    def forward(self):
        ...
        temp2 = torch.mul(temp1, x1)
        temp3 = torch.mul(temp2, x2)
        result1 = torch.mul(temp2, self.InData1)
...
class EF(nn.Module):
    ...
    def __init__(self, net1, net2):
        ...
        # 将resnet的1024通道改为2048便于add融合
        self.conv1 = nn.Conv2d(in_channels=1024, out_channels=2048, kernel_size=1, stride=1, bias=False)

        self.avgpool = nn.Sequential(nn.AdaptiveAvgPool2d((1, 1)))
        # self.fc = nn.Sequential(nn.Linear(3072, 30))

        # add的代码
        self.fc = nn.Sequential(nn.Linear(2048, 30))

    def forward(self, input):
        f1 = self.net1(input)  # 7*7*2048
        f2 = self.net2(input)  # 7*7*1024
        f2 = self.conv1(f2)  # 7*7*2048
...
def main(args):
    ...
    epochs = 100
    best_acc = 0.0
    save_path = './AIMAX_D_R_add_1024_2048.pth'
    ...
    for epoch in range(epochs):
        ...
        with open('./Draw/val_accurate_D_R_add_1024_2048.txt', 'a+') as f:
            f.write(str(val_accurate) + ',')

        train_loss1 = running_loss / train_steps
        with open('./Draw/train_loss_D_R_add_1024_2048.txt', 'a+') as f:
