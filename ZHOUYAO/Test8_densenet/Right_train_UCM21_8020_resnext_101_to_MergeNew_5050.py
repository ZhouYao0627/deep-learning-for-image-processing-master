...
# best_acc 0.9904761904761905 -> 98.97
def main():
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    print("using {} device.".format(device))

    print('Start Tensorboard with "tensorboard --logdir=runs", view at http://localhost:6006/')
    tb_writer = SummaryWriter(log_dir="runs/UCM21_experiment1")
    ...
    epochs = 100  # 原来是3
    best_acc = 0.0
    save_path = './save_weights/train_UCM21_8020.pth'
    ...
    for epoch in range(epochs):
        ...
        with open('./Draw/train_accurate_UCM21_8020.txt', 'a+') as f:
            f.write(str(train_accurate) + ',')

        with open('./Draw/val_accurate_UCM21_8020.txt', 'a+') as f:
            f.write(str(val_accurate) + ',')

        train_loss1 = running_loss / train_steps
        with open('./Draw/train_loss_UCM21_8020.txt', 'a+') as f:
