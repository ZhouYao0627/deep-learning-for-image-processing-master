...
class mxl(nn.Module):
    ...
    def __init__(self, input1, input2):
        ...
        self.conv_1 = nn.Conv2d(in_channels=4096, out_channels=2048, kernel_size=1, bias=False)
        # self.conv_2 = nn.Conv2d(in_channels=32, out_channels=16, kernel_size=1, bias=False)

    # 前向传播
    def forward(self):
        x1 = self.input2 * self.sigmoid(self.avg_pool(self.input1))
        x2 = self.input1 * self.sigmoid(self.avg_pool(self.input2))

        # x3 = self.conv_1(x1)
        # x4 = self.conv_1(x2)


        x7 = torch.cat((self.input1, self.input2 ), dim=1)  # 4096*d
        # x5=x4*x4
        # x6=torch.sigmoid(x5)
        # x7=x6*x4
        #x5 = self.conv_1(x4)  # 7*7*2048
        #x5=self.avg_pool(x4)
        #x5 = torch.mean(x4)
        #x5=self.conv_1(x4)
        #x6 = self.sigmoid(x5)

        #x6 = x6.permute(0,2,3,1)
        #print(x6.shape,x4.shape)
        #x7 = x4 * x6 # 4096*2048
...
class EF(nn.Module):
    ...
    def __init__(self, net1, net2):
        ...
        # 3x3卷积
        self.conv_3x3_1 = nn.Conv2d(in_channels=4096, out_channels=1024, kernel_size=3, stride=1, padding=1, bias=False)
        ...
        self.conv_1_1 = nn.Conv2d(in_channels=2208, out_channels=2048, kernel_size=1, bias=False)
    ...
    def forward(self, input):
        ...
        net = net.to(device)
        x = net.forward()
        # x = mxl(f1, f2).to(device)

        # x = net.forward()  # 7*7*2018
...
def main(args):
    ...
    epochs = 100
    best_acc = 0.0
    save_path = './save_weights/D_R_UCM21_9010_Merge_0dropout1.pth'
    ...
    for epoch in range(epochs):
        ...
        # with open('./Draw/train_accurate_UCM21_9010_Merge_0dropout.txt', 'a+') as f:
        #     f.write(str(train_accurate) + ',')
        #
        # with open('./Draw/val_accurate_UCM21_9010_Merge_0dropout.txt', 'a+') as f:
        #     f.write(str(val_accurate) + ',')
        #
        # train_loss1 = running_loss / train_steps
        # with open('./Draw/train_loss_UCM21_9010_Merge_0dropout.txt', 'a+') as f:
