...
class spatial_attention(nn.Module):
    ...
    def __init__(self, kernel_size=7):
        ...
        # 反卷积复原特征图大小 [b,4096,h/2,w/2]==>[b,1,h,w]
        self.deconv = nn.ConvTranspose2d(in_channels=1, out_channels=1, kernel_size=1, stride=2, padding=0,
                                         bias=False)
        # sigmoid函数
        self.sigmoid = nn.Sigmoid()
    ...
    def forward(self, inputs):
        ...
        # print("outputs3.shape", outputs3.shape)

        # 对特征图进行反卷积操作 [b,1,h/2,w/2]==>[b,1,h,w]
        outputs4 = self.deconv(outputs3)
        # print("outputs4.shape", outputs4.shape)
