...
from pytorch_classification.Test1_official_demo.model import LeNet
...
def main():
    ...
    for epoch in range(5):  # loop over the dataset multiple times
        ...
        for step, data in enumerate(train_loader, start=0):  # 遍历训练集样本
            ...
            loss = loss_function(outputs, labels)  # 计算当前损失
            ...
            if step % 500 == 499:  # print every 500 mini-batches
                with torch.no_grad():
                    ...
                    predict_y = torch.max(outputs, dim=1)[1]  # troch.max()[1]， 只返回最大值的每个索引
