...
class ConfusionMatrix(object):
    ...
    def plot(self):  # 绘制混淆矩阵
        matrix = self.matrix
        print(matrix)  # 打印混淆矩阵
        plt.imshow(matrix, cmap=plt.cm.Blues)  # 展示混淆矩阵；颜色从白色->蓝色
        # plt.imshow()函数负责对图像进行处理，并显示其格式，但是不能显示。其后跟着plt.show()才能显示出来。

        # 设置x轴坐标label
        plt.xticks(range(self.num_classes), self.labels, rotation=45)  # x轴的信息替换成labels，旋转45°
        # 设置y轴坐标label
...
if __name__ == '__main__':
    ...
    confusion = ConfusionMatrix(num_classes=19, labels=labels)
    net.eval()  # 验证模式
