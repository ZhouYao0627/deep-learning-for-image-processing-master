...
class Attention(nn.Module):
    def __init__(self,
                 dim,  # 输入token的dim   -> 设定的总编码的大小，这里为768
                 num_heads=8,
                 qkv_bias=False,
                 qk_scale=None,
                 attn_drop_ratio=0.,  # q乘k的转置除以根号dk后通过softmax之后的dropout
                 proj_drop_ratio=0.):  # Muti-Attention中最后一个全连接层后使用的dropout
        super(Attention, self).__init__()
        self.num_heads = num_heads
        head_dim = dim // num_heads  # 768//8=96
        self.scale = qk_scale or head_dim ** -0.5  # 即相当于根号下dk分之1
        self.qkv = nn.Linear(dim, dim * 3, bias=qkv_bias)  # qkv的生成，使用一个全连接层一次性得到qkv
        self.attn_drop = nn.Dropout(attn_drop_ratio)
        self.proj = nn.Linear(dim, dim)
        self.proj_drop = nn.Dropout(proj_drop_ratio)

    def forward(self, x):
        # [batch_size, num_patches + 1, total_embed_dim]  ->  [batch_size, 197, 768]
        B, N, C = x.shape
        ...
        attn = attn.softmax(dim=-1)  # 针对每一行进行处理
        ...
        x = (attn @ v).transpose(1, 2).reshape(B, N, C)  # 这里的reshape相当于concat拼接
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
...
class Block(nn.Module):
    def __init__(self,
                 ...
                 qkv_bias=False,
                 qk_scale=None,
                 drop_ratio=0.,  # Muti-Attention中最后一个全连接层后使用的dropout
                 attn_drop_ratio=0.,  # q乘k的转置除以根号dk后通过softmax之后的dropout
                 drop_path_ratio=0.,  # Encoder Block中的dropout
                 act_layer=nn.GELU,
...
class VisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_c=3, num_classes=1000,
        ...
        Args:
            ...
            depth (int): depth of transformer  -> 重复堆叠block的次数
            num_heads (int): number of attention heads
            mlp_ratio (int): ratio of mlp hidden dim to embedding dim
            qkv_bias (bool): enable bias for qkv if True
            qk_scale (float): override default qk scale of head_dim ** -0.5 if set
            representation_size (Optional[int]): enable and set representation layer (pre-logits) to this value if set
            distilled (bool): model includes a distillation token and head as in DeiT models  -> 为了兼容Deit模型
        ...
        self.num_tokens = 2 if distilled else 1  # 不使用Deit模型，故默认为1
    ...
    def forward_features(self, x):
        ...
        x = self.pos_drop(x + self.pos_embed)  # 将Position Embedding加进来
        x = self.blocks(x)
        x = self.norm(x)
        if self.dist_token is None:
